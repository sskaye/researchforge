...
Also catches the specific symptom that bit Trial-1-full: the `meas_calc`
...
_ALLOWED_MEAS_CALC = {"measured", "calculated", ""}
...
def main():
    ...
    # Find the meas_calc column index
    try:
        dt_idx = header.index("meas_calc")
    except ValueError:
        dt_idx = None
    ...
    for li, row in enumerate(rows[1:], start=2):
        ...
        if len(row) != n_expected:
            ...
            flagged += 1
            continue
        # Check meas_calc sanity (if header has it)
        if dt_idx is not None:
            dt = (row[dt_idx] or "").strip()
            if dt not in _ALLOWED_MEAS_CALC:
                print(f"row {rid}: meas_calc={dt!r} not in "
                      f"{sorted(_ALLOWED_MEAS_CALC)} — likely column shift "
