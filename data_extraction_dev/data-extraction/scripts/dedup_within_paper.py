...
Two rows are "duplicates within paper" if they share:
  ...
  - property
  - value (within tolerance; default 0.5 — pass --tolerance to override for
    properties whose values use a different scale)
...
Different `meas_calc` (measured vs. calculated) for the same compound /
...
def main():
    ...
    ap.add_argument("--tolerance", type=float, default=0.5,
                    help="Numeric tolerance for value match (default 0.5; "
                         "appropriate for °C; override for other unit scales)")
    args = ap.parse_args()
    ...
    with open(args.csv_path, newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            ...
            key = (
                ...
                (row.get("meas_calc") or "").strip().lower(),
            )
