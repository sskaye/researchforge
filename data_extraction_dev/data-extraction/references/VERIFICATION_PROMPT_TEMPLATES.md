...
Q1(b) (value-matches-paper) and Q2 (property-type-correct) are the two questions that customize per data type — they reference the overlay's single `OVERLAY.md` file (which contains the property enum, standardized unit, value ranges, and worked examples). Q1(a), Q3, Q4, and Q5 are property-agnostic.
...
>
> **Data type:** `{datatype}`. Read `datatypes/{datatype}/OVERLAY.md` end-to-end — this single file contains the property enum (including property-disambiguation table), the standardized unit string for `units`, extension columns, value ranges, unit conversions, and the property-specific failure-mode catalog. If the overlay folder doesn't exist, stop and ask — do not attempt verification without it.
>
> **Row to verify:**
>
> ```
...
> (plus any overlay extension columns)
> ```
>
...
>
> **(b) Is the value (`value`, `value_min`/`value_max`, `value_raw`, `units`) the value the paper reports for THIS compound?**
>
> Pass:
> - Within the paper's stated precision.
> - Range shorthand expansion is fine (paper "237-39" = row "237-239").
> - Unit conversion arithmetically correct (see `datatypes/{datatype}/OVERLAY.md` § Unit conversions for the property's standardized unit and conversion examples).
>
...
>
> Is `property` the physical phenomenon the paper actually reports for this value? Allowed values are declared in `datatypes/{datatype}/OVERLAY.md` § Properties (e.g., for mp_bp: `melting_point`, `boiling_point`, `DSC_onset`, `DSC_peak`, `decomposition`, `sublimation`). The OVERLAY also carries a property-disambiguation table for borderline cases (e.g., for mp_bp: "onset of decomposition" → `decomposition`, not `DSC_onset`; `"X °C, decomp."` → `decomposition`, not `melting_point`).
>
> Fail (`flagged_property_subtype_mismatch`):
> - The property field names a subtype the paper doesn't actually use (e.g., `melting_point` for a value the paper labels "decomposition", or `DSC_peak` for a paper-labeled "DSC onset").
>
> ---
>
> ## Q3 — meas_calc correctness (Tier 1, per schema)
>
> The `meas_calc` column has two values: `measured` and `calculated`.
>
...
>
> A review-paper compilation of literature mp values labeled `measured` PASSES. A QSPR "Exp." column labeled `measured` PASSES. A QSPR "Calc." column labeled `measured` FAILS (`flagged_meas_calc_mismatch`).
>
...
- ❌ "The paper is a review and labels this `measured`, fail it as wrong meas_calc" — **NO.** The schema's `measured` = any experimental observation, including literature compilations. Pass Q3.
