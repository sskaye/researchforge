#!/usr/bin/env python3
"""
voltage_range_check.py — flag rows whose value is outside the plausible range
for the chemical class indicated in the compound_name field (with notes as a fallback signal).
...
Class detection priority:
    1. Match against the `compound_name` field. Use the most specific keyword that matches
       (e.g., "anthraquinone" wins over "quinone").
    2. Only fall back to `notes` if the compound_name field provides no clue (e.g., when the
       compound_name field is a SMILES).
...
def detect_class(row):
    ...
    Match priority: compound_name field (primary) > notes field (fallback). Only the
    first matching class is returned.
    """
    compound_name = (row.get("compound_name") or "").lower()
    ...
    # Try compound_name field first
    for kw, sc, lo, hi in RANGES:
        if kw in compound_name and sc == sclass:
        ...
        for row in reader:
            ...
            if v < lo or v > hi:
                ...
                if fail <= args.max_output:
                    ...
                    print(f"   compound_name: {row.get('compound_name', '')[:60]}")
