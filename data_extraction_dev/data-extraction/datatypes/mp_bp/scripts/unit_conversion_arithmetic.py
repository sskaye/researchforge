...
Rules (using v2.0 standardized conversion-arithmetic syntax — see SKILL.md):
  ...
  - If `value_raw` is in °F, expect `(<X> °F − 32) × 5/9 = <Y> °C` and
    confirm Y ≈ (X − 32) × 5/9.
  - Both ASCII (`-`, `*`) and unicode (`−`, `×`) operators are accepted as input
    so older v1.7-era CSVs can still be linted; new output should use unicode.
  - If `value_raw` is already in °C, `conversion_arithmetic` must be empty.
  - In every case, the row's `value` field must match the computed
...
def check_row(row: dict) -> str | None:
    ...
    try:
        ...
        return f"value={canon!r} is not a number"
    ...
    if canon_val is None:
        return f"value is empty but value_raw is {raw!r}"
    if abs(canon_val - expected) > TOLERANCE_C:
        return (f"value={canon_val} disagrees with expected "
