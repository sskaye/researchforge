...
Output:
    Lines like:
      row 42: value=4823.0 °C exceeds mp upper bound 4500
...
def check_row(row: dict) -> str | None:
    ...
    if units and units not in ("°C", "C"):
        return (f"units={units!r} unexpected for mp/bp data (mp_bp overlay "
                "uses °C exclusively); skipping range check")
    ...
    lo, hi = rng
    if val < lo:
        return (f"value={val} °C below {prop} lower bound {lo} — "
                "check for PDF sign-loss artifact")
    if val > hi:
        return (f"value={val} °C exceeds {prop} upper bound {hi} — "
